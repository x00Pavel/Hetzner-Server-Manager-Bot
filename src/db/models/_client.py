...
class Client(Base):
    ...
    @classmethod
    async def get_by_id(cls, db: AsyncSession, id: int) -> "Client | None":
        result = await db.execute(select(cls).where(cls.id == int(id)))
        return result.scalars().first()

    @classmethod
    async def get_by_remark(cls, db: AsyncSession, remark: str) -> "Client | None":
        result = await db.execute(select(cls).where(cls.remark == remark))
        return result.scalars().first()

    @classmethod
    async def get_by_secret(cls, db: AsyncSession, secret: str) -> "Client | None":
